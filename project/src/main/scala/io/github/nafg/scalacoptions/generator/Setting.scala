...
case class Setting(
  ...
  def mergeLiteralSegments =
    copy(flagSegments = flagSegments.foldRight(List.empty[FlagSegment]) {
      case (
            FlagSegment.Literal(text1),
            FlagSegment.Literal(text2) :: segments
          ) =>
...
object Setting {
  implicit val settingLListIso: IsoLList.Aux[Setting, String :*: Seq[
    FlagSegment
  ] :*: String :*: Boolean :*: LNil] =
    LList.iso(
      ...
      {
        case (_, name) :*: (_, flagSegments) :*: (_, description) :*: (
              _,
              isDeprecated
            ) :*: LNil =>
